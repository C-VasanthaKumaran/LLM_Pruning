...
if __name__ == "__main__":
    ...
    parser.add_argument('--base_model', type=str, default="TinyLlama/TinyLlama_v1.1", help='base model name')
    parser.add_argument('--model_type', type=str, required=True, help = 'choose from [pretrain, pruneLLM]')
    parser.add_argument('--ckpt', type=str, default=None)
    parser.add_argument('--lora_ckpt', type=str, default=None)
    
    args = parser.parse_args()
    pruned_dict = torch.load('/home/vasanth/Downloads/LLM-Pruner-main/prune_log/tinyllama_prune_log/pytorch_model.bin', map_location='cpu')
    tokenizer, model = pruned_dict['tokenizer'], pruned_dict['model']
    main(args)
'''
    ...
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(model, (1, 64,), as_strings=True,
                                                  ...
                                                  custom_modules_hooks={
                                                      LlamaAttention: LlamaAttention_counter_hook,
                                                      LlamaRMSNorm: rmsnorm_flops_counter_hook,
                                                      # Removed SiLUActivation import as it's not needed
                                                      pool_flops_counter_hook: pool_flops_counter_hook,
                                                  })
...
if __name__ == "__main__":
    ...
    parser.add_argument('--base_model', type=str, default="TinyLlama/TinyLlama-1.1B-Chat-v1.0", help='base model name')
    parser.add_argument('--model_type', type=str, required=True, help = 'choose from [pretrain, pruneLLM]')
    parser.add_argument('--ckpt', type=str, default=None)
    parser.add_argument('--lora_ckpt', type=str, default=None)
    ...
    pruned_dict = torch.load('/home/vasanth/Downloads/LLM-Pruner-main/prune_log/tinyllama_chat_prune_log/pytorch_model.bin', map_location='cpu')
    tokenizer, model = pruned_dict['tokenizer'], pruned_dict['model']
